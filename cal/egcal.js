				...
				if (eventsArray[i-n] != null)
				{
					if (eventsArray[i-n].length >= r+1)
					{
							...
							//else if (eventsArray[i-n][r].startDate.getTime() == (w.getTime() - EGCalendarFundamental.oneday*(n+1)) &&eventsArray[i-n][eventsArray[i-n].length-1].rank > r)
								...
								(eventsArray[i-n][r].startDate.getTime() != (w.getTime() - EGCalendarFundamental.oneday*(n+1)) && eventsArray[i-n][r].rank > r)
								)
//							if (false)
							{
								if (r == re)
								...
								if (this.debug)
									ret += '<td class="' + cellClass + '" >c ' + "," + n + "," +  r + "," +  eventsArray[i-n][r].rank + ' </td>';						
								else
						...
						if (this.debug)
							ret += '<td class="' + cellClass + '" >b ' +  "," + n + "," + eventsArray[i-n][eventsArray[i-n].length-1].rank + ' </td>';
						else
							ret += '<td class="' + cellClass + '" ></td>';
					}				
				} else
...
EGCalendarFundamental.prototype.gcalendar = function(targetFrame, events) {
	...
	var header = '<head><title></title>\n'
	    + '<link href="http://www.google.com/calendar/3d6b50f6c45e613af5d7d1e8e686aa58doozercompiled.css" type="text/css" rel="stylesheet">\n'		
			+ '<link href="http://www.google.com/calendar/embed/3d6b50f6c45e613af5d7d1e8e686aa58embedcompiled.css" type="text/css" rel="stylesheet">\n'
